...
/**
 * Created by liujianghao on 16-4-15.
 */
public class FanProgressBar extends View {
    ...
    private static final int DEFAULT_OUT_RADIUS = 50;
    ...
    public FanProgressBar(Context context, AttributeSet attrs) {
        super(context, attrs);
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.FanProgressBar);
        try {
            mMaxProgress = typedArray.getFloat(R.styleable.FanProgressBar_circleMaxProgress, DEFAULT_MAX_PROGRESS);
            mProgress = typedArray.getFloat(R.styleable.FanProgressBar_circleProgress, 0);
            mInternalRadius = typedArray.getInt(R.styleable.FanProgressBar_internalRadius, DEFAULT_INTERNAL_RADIUS);
            mOutRadius = typedArray.getInt(R.styleable.FanProgressBar_outRadius, DEFAULT_OUT_RADIUS);
            mInternalBackgroundColor = typedArray.getColor(R.styleable.FanProgressBar_internalBackgroundColor, Color.WHITE);
            mOutBackgroundColor = typedArray.getColor(R.styleable.FanProgressBar_outBackgroundColor, Color.WHITE);
            mStartAngle = typedArray.getFloat(R.styleable.FanProgressBar_startAngle, DEFAULT_START_ANGLE);
            mInternalAlpha = (int) (255 * typedArray.getFloat(R.styleable.FanProgressBar_internalAlpha, DEFAULT_INTERNAL_ALPHA));
            mOutAlpha = (int) (255 * typedArray.getFloat(R.styleable.FanProgressBar_outAlpha, DEFAULT_OUT_ALPHA));
            mCenterX = typedArray.getFloat(R.styleable.FanProgressBar_centerX, Integer.MAX_VALUE);
            mCenterY = typedArray.getFloat(R.styleable.FanProgressBar_centerY, Integer.MAX_VALUE);
            mOutDirection = typedArray.getInt(R.styleable.FanProgressBar_outDirection, DIRECTION_CLOCKWISE);
            mInternalDirection = typedArray.getInt(R.styleable.FanProgressBar_internalDirection, DIRECTION_CLOCKWISE);
            mInitStyle = typedArray.getInt(R.styleable.FanProgressBar_initStyle, EMPTY_INIT);
        } finally {
            typedArray.recycle();
        }
        initPaint();
        ...
        if (progress == mMaxProgress) {
            ...
            if (sweepAngle != 0) {
                ...
                if (mOutRadius > 0) {
                    ...
                    ovalOut.left = mCenterX - mOutRadius;
                    ovalOut.right = mCenterX + mOutRadius;
                    ovalOut.top = mCenterY - mOutRadius;
                    ovalOut.bottom = mCenterY + mOutRadius;
                    canvas.drawArc(ovalOut, mStartAngle, outSweepAngle, true, mOutPaint);
                }
