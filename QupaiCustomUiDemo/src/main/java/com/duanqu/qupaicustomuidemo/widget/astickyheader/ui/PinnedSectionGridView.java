...
public class PinnedSectionGridView extends AutoScrollGridView {
	...
	void createPinnedShadow(int position) {
		...
		// read overlay_time_edit_layout parameters
		AbsListView.LayoutParams layoutParams = (AbsListView.LayoutParams) pinnedView.getLayoutParams();
		if (layoutParams == null) { // create default overlay_time_edit_layout params
		...
		// measure & overlay_time_edit_layout
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        if (mNumColumns == GridView.AUTO_FIT) {
        	mAvailableWidth = View.MeasureSpec.getSize(widthMeasureSpec);
            if (mColumnWidth > 0) {
            	int availableSpace = View.MeasureSpec.getSize(widthMeasureSpec) - getPaddingLeft() - getPaddingRight();
